...
模型、机制和算法必须分层：混合整数线性规划、动态回归、车辆路径、状态空间等是模型族；维护约束、预算平衡、滚动时域、超边修复是定制机制；HiGHS、Gurobi、分支定界、遗传算法、粒子群、匈牙利算法等是求解算法。多指标决策方法可登记为标准模型族时，必须同时写出指标、权重、归一化和聚合/排序关系，不能只报方法名称。

MCM/ICM 使用：`MODEL DEFINITION Q1 | ACADEMIC NAME: ... | CANONICAL MODEL FAMILY: ... | SOLVER ALGORITHM: ...` 与 `MODEL STRUCTURE Q1 | DECISION/STATE VARIABLES: ... | OBJECTIVE/STATISTICAL RELATION: ... | CORE CONSTRAINTS/EQUATIONS: ... | CUSTOM MECHANISM: ...`。
