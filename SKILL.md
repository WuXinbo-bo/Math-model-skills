...
- FORMULATION 的每个子问题必须在现有 `建模报告.md` 中登记两行模型身份卡：`模型定义 QN | 正式名称: ... | 标准模型族: ... | 求解算法: ...` 与 `模型结构 QN | 决策变量/状态量: ... | 目标函数/统计关系: ... | 核心约束/方程: ... | 定制机制: ...`。不新增文件。
...
- MANUSCRIPT 每问按“模型名称与建模思想 → 假设、变量与数学表达 → 求解算法 → 结果与检验 → 模型解释与边界”组织；正式名称、标准模型族和求解算法必须与 FORMULATION 身份卡一致。

### 摘要内容合同

- 中文竞赛摘要按五段组织：首段说明问题、困难、建立的标准模型及任务；中间每个子问题一段，按“正式模型名称与标准模型族 → 定制机制 → 求解算法 → 关键结果 → 检验”表述；尾段总结模型的可解释性、稳健性、推广性和局限性。
