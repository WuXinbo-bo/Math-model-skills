...
def make_s6(workspace: Path) -> None:
    ...
    write_text(
        sections / "2_模型构建.tex",
        "问题一建立考虑容量与预算约束的多目标优化线性规划模型，标准模型族为多目标优化，采用加权和法与HiGHS算法求解。\n"
        "问题二建立考虑阶段转移与情景扰动的鲁棒优化模型，标准模型族为鲁棒优化，采用情景生成与HiGHS算法求解。\n"
        "问题三建立兼顾公平与效率的TOPSIS多指标决策模型，标准模型族为多指标决策，采用熵权TOPSIS排序算法求解。\n"
        + (
    ...
    write_text(
        ...
        textwrap.dedent(
            ...
            \section*{摘要}
            \label{AbstractStart}
            本文针对复杂系统资源配置、阶段扰动与综合评价问题，建立三类标准数学模型，完成约束优化、风险控制和方案排序任务。

            针对问题一，建立考虑容量与预算约束的多目标优化线性规划模型，标准模型族为多目标优化，采用加权和法与HiGHS算法求解，得到最优目标值10，并通过预算约束回代验证可行性。

            针对问题二，建立考虑阶段转移与情景扰动的鲁棒优化模型，标准模型族为鲁棒优化，采用情景生成与HiGHS算法求解，得到风险暴露值20，并通过灵敏度检验验证结果稳健性。

            针对问题三，建立兼顾公平与效率的TOPSIS多指标决策模型，标准模型族为多指标决策，采用熵权TOPSIS排序算法求解，得到综合绩效得分30，并通过权重扰动检验排序稳定性。
